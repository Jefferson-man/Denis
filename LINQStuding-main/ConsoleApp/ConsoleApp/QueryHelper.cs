...
public class QueryHelper : IQueryHelper
{
    ...
    public IEnumerable<Delivery> NotFinished(IEnumerable<Delivery> deliveries) =>
        deliveries.Where(x =>
            x.Status != DeliveryStatus.Done && x.Status != DeliveryStatus.Cancelled); //TODO: Завдання 2
    ...
    public IEnumerable<DeliveryShortInfo> DeliveryInfosByClient(IEnumerable<Delivery> deliveries, string clientId) =>
        deliveries.Where(x => x.ClientId == clientId).Select(x => new DeliveryShortInfo
        {
            Id = x.Id,
    ...
    public IEnumerable<Delivery>
        DeliveriesByCityAndType(IEnumerable<Delivery> deliveries, string cityName, DeliveryType type) =>
        deliveries.Where(x => x.Direction.Origin.City == cityName && x.Type == type).Take(10); //TODO: Завдання 4
    ...
    public Dictionary<DeliveryStatus, int> CountsByDeliveryStatus(IEnumerable<Delivery> deliveries) =>
        deliveries.GroupBy(x => x.Status).ToDictionary(group => group.Key, group => group.Count()); //TODO: Завдання 7
    ...
    public IEnumerable<AverageGapsInfo> AverageTravelTimePerDirection(IEnumerable<Delivery> deliveries)=> 
        deliveries.GroupBy(x => new { StartCity = x.Direction.Origin.City, EndCity = x.Direction.Destination.City }).
     ...
     public IEnumerable<TElement> Paging<TElement, TOrderingKey>(IEnumerable<TElement> elements,
         ...
         int countOnPage = 100,
         int pageNumber = 1) =>
         elements.Where(filter).OrderBy(ordering).Skip((pageNumber - 1) * countOnPage).Take(countOnPage);    //new List<TElement>(); //TODO: Завдання 9 
}